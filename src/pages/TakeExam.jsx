    ...
    return (<div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="flex gap-1 mb-6">
          {exam.questions.map((_, i) => (<div key={i} className={cn("h-2 flex-1 rounded-full cursor-pointer transition-colors", i === current ? "bg-primary" : answers[exam.questions[i].id] ? "bg-primary/40" : "bg-muted")} onClick={() => setCurrent(i)}/>))}
        </div>
        ...
        <div className="space-y-3 mb-8">
          {q.question_type === "coding" ? (<>
              {q.code_template && (<div className="rounded-xl border border-border bg-muted/40 p-4">
              ...
              <Textarea value={answers[q.id] || ""} onChange={(e) => setAnswers({ ...answers, [q.id]: e.target.value })} className="min-h-[260px] font-mono" placeholder="Write your solution here..."/>
            </>) : ([
            { key: "a", text: q.option_a },
            { key: "b", text: q.option_b },
            { key: "c", text: q.option_c },
            { key: "d", text: q.option_d },
        ].map((opt) => (<button key={opt.key} onClick={() => setAnswers({ ...answers, [q.id]: opt.key })} className={cn("w-full text-left p-4 rounded-xl border transition-all", answers[q.id] === opt.key
                ? "border-primary bg-primary/5 ring-2 ring-primary/20"
                : "border-border hover:border-primary/30 hover:bg-muted/50")}>
                <span className={cn("inline-flex items-center justify-center h-6 w-6 rounded-full text-xs font-bold mr-3", answers[q.id] === opt.key ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground")}>
                  {opt.key.toUpperCase()}
                </span>
        ...
        <div className="flex gap-3">
          ...
          {current < exam.questions.length - 1 ? (<Button onClick={() => setCurrent(current + 1)} className="gap-1">Next <ArrowRight className="h-4 w-4"/></Button>) : (<Button onClick={handleSubmit} disabled={submitting} className="hero-gradient border-0 text-primary-foreground gap-1">
