          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {certs.map((c) => (
              <div key={c.id} className="stat-card">
                ...
                {!c.is_payment_completed ? (
                  <Button size="sm" className="w-full gap-2" onClick={() => handlePay(c.id)} disabled={payingId === c.id}>
                    <CreditCard className="h-3 w-3" /> {payingId === c.id ? "Processing..." : "Pay & Unlock"}
                  </Button>
                ) : (
                  <div className="space-y-2">
                    {c.is_approved && (
                      <Button size="sm" className="w-full gap-2" onClick={() => handleDownload(c.id)}>
                    ...
                    {(() => {
                      ...
                      return payment ? (
                        <Button size="sm" variant="outline" className="w-full gap-2" onClick={() => handleInvoice(payment.id)}>
